...
const basePath = config.basePath || '/api/brand-manager';
...
// Apply rate limiter to all API routes under basePath
...
app.post(`${basePath}/crawl`, async (req, res) => {
...
app.get(`${basePath}/sites/:id`, async (req, res) => {
  const { id } = req.params;
...
app.get(`${basePath}/sites/:id/tokens`, async (req, res) => {
  const { id } = req.params;
...
app.get(`${basePath}/sites/:id/brand-profile`, async (req, res) => {
  const { id } = req.params;
